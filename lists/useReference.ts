...
import { useGetManyAggregate, UseGetManyHookValue } from '@specfocus/view-focus.data/operations';
import { UseQueryOptions } from 'react-query';

interface UseReferenceProps<RecordType extends Entity = any> {
  id: Identifier;
  reference: string;
  options?: UseQueryOptions<RecordType[], Error>;
}

export interface UseReferenceResult<RecordType extends Entity = any> {
  isLoading: boolean;
  isFetching: boolean;
 ...
 */
export const useReference = <RecordType extends Entity = any>({
  reference,
  id,
  options,
